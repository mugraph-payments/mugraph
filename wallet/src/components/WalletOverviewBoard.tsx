import { ArrowClockwise, ArrowSquareOut, Coins, Stack } from "@phosphor-icons/react";
import type { ReactNode } from "react";
import type { WalletActivityView, WalletAssetView, WalletNoteView } from "../lib/walletView";
    ...
    <section className="wallet-card p-5">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <h3 className="wallet-heading mt-2 text-lg font-semibold text-slate-50">{title}</h3>
        </div>
      </div>
...
export function WalletOverviewBoard({ assets, notes, activity }: WalletOverviewBoardProps) {
    ...
    <section className="wallet-panel p-5 sm:p-6">
      <div className="flex flex-col gap-2 xl:flex-row xl:items-end xl:justify-between">
        ...
        <p className="wallet-copy max-w-2xl text-base leading-8 text-slate-400">
          The overview keeps the portfolio, spendable note inventory, and live queue in one place so
          you can see the wallet posture before taking an action.
        </p>
      </div>
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-2 2xl:grid-cols-3">
        ...
        >
          <div className="grid gap-3">
            {latestNotes.map((note) => (
              <div key={note.id} className="wallet-subtle-card flex flex-col gap-3 p-3">
        ...
        >
          <div className="grid gap-3">
            {queueItems.map((item) => (
              <div key={item.id} className="wallet-subtle-card flex flex-col gap-3 p-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
