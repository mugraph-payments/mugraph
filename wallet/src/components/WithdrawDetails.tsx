  ...
  return (
    <div className="mt-4 rounded-[1.5rem] border border-white/10 bg-white/[0.02] p-4">
      <p className="text-sm leading-6 text-slate-300">
        Withdrawal prep keeps the most recent settlement reference, current
        queue pressure, and the active script context visible while notes are
        being assembled for on-chain settlement.
      </p>

      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <DetailField label="Latest withdraw ref" value={latestWithdrawReference} />
        <DetailField
          label="Pending queue"
          value={`${pendingActivityCount} item${pendingActivityCount === 1 ? "" : "s"}`}
        />
        <DetailField label="Settlement path" value={scriptAddressShort} />
        <DetailField label="Primary asset in view" value={topAssetLabel} />
      </div>
    </div>
  );
}
