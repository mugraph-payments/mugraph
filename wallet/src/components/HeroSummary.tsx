...
import type { WalletIdentityView, WalletSummaryMetricView, WalletTone } from "../lib/walletView";
...
export function HeroSummary({ identity, summaryMetrics }: HeroSummaryProps) {
  ...
  return (
    <section className={`wallet-panel p-5 sm:p-6 ${tone.shell}`}>
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(16rem,0.9fr)]">
        <div className="min-w-0 space-y-5">
          <div className="space-y-2">
            <p className={`wallet-kicker ${tone.eyebrow}`}>Wallet posture</p>
            <h2 className="wallet-heading text-3xl font-semibold tracking-tight text-slate-50 sm:text-5xl sm:leading-tight">
              Private balances and settlement flow in one focused workspace
            </h2>
            <p className={`wallet-copy max-w-2xl text-base leading-8 ${tone.copy}`}>
              This redesign treats the wallet like an operator cockpit instead of a dashboard: the
              current value, inventory health, and next action all stay visible without crowding the
              screen.
            </p>
          </div>
          ...
          <div className="wallet-panel-soft p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="wallet-data mt-3 text-5xl font-semibold tracking-tight text-slate-50 sm:text-6xl">
                  {totalAda?.value}
                </p>
                <p className="wallet-data mt-2 text-lg text-slate-300">{totalUsd?.value}</p>
              </div>
          ...
          <div className="wallet-card p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className={`wallet-data mt-2 text-3xl font-semibold ${tone.accent}`}>
                  {noteCount?.value}
                </p>
              </div>
